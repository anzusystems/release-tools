...
export class GitHub {
  ...
  async mergeRequirements() {
    ...
    const rules = (await this.request('GET', `${this.base}/rules/branches/main`)).data ?? []
  ...
  /**
   * An annotated tag created through the API (mock registry only).
   * @param {string} name
   * @param {string} commit
   * @param {string} message
   * @param {boolean} force
   */
  async createApiTag(name, commit, message, force = false) {
    this.mutate(`create the tag ${name} on GitHub`)
    const obj = (await this.request('POST', `${this.base}/git/tags`, { body: { tag: name, message, object: commit, type: 'commit' } })).data
    if (force) {
