...
/**
 * A POST refused with 422 after an earlier attempt failed may have been done by that attempt: `find` looks for
 * its result.
 * @template T
 * @param {unknown} e
 * @param {() => Promise<T>} find
 * @returns {Promise<T | null | false>}
 */
async function doneBefore(e, find) {
  if (!(e instanceof GitHubError) || e.status !== 422 || !e.retried) return null
  return find()
}
...
export class GitHubError extends Error {
  ...
  constructor(message, status, data) {
    ...
    this.status = status
    this.data = data
    /** an earlier attempt of the same request failed (a POST may have been done already) */
    this.retried = false
  }
}
...
export class GitHub {
   ...
   * @param {{ body?: any, raw?: Buffer, contentType?: string, allow404?: boolean, emptyList?: boolean, accept?: string, retries?: number }} [o]
   *   emptyList: a GET of a list in an empty repository is an empty list
  ...
  async request(method, path, o = {}) {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      // An empty repository has no refs at all; GitHub answers 409 where it answers 404 otherwise.
      const empty = method === 'GET' && res.status === 409 && /repository is empty/i.test(String(data?.message ?? ''))
      if ((res.status === 404 || empty) && o.allow404) return { status: 404, headers: res.headers, data: null }
      if (empty && o.emptyList) return { status: 200, headers: res.headers, data: [] }
      if (res.status >= 400) {
        const detail = data?.errors ? ` ${JSON.stringify(data.errors)}` : ''
        const error = new GitHubError(`${method} ${path}: ${res.status} ${data?.message ?? text}${detail}`, res.status, data)
  ...
  async paginate(path, key = null) {
    ...
    while (url) {
      const r = await this.request('GET', url, { emptyList: true })
  ...
  async createBranch(name, sha) {
    ...
    await this.request('POST', `${this.base}/git/refs`, { body: { ref: `refs/heads/${name}`, sha } }).catch(async (e) => {
      if (!(await doneBefore(e, async () => (await this.branchSha(name)) === sha))) throw e
    })
  }
  ...
  async createApiTag(name, commit, message, force = false, tagger) {
    ...
    await this.request('POST', `${this.base}/git/refs`, { body: { ref: `refs/tags/${name}`, sha: obj.sha } }).catch(async (e) => {
      if (!(await doneBefore(e, async () => (await this.tag(name))?.refSha === obj.sha))) throw e
    })
    return obj.sha
  }
  ...
  async createPull(o) {
    ...
    try {
      ...
      const open = await doneBefore(e, async () => (await this.pulls({ head: o.head, base: o.base, state: 'open' }))[0] ?? null)
      if (open) return open
      throw e
    }
  }
  ...
  /**
   * The state of a workflow (active, disabled_manually, disabled_inactivity, …); null when GitHub does not know it
   * yet (a workflow file only in a branch).
   * @param {string} workflow file name
   * @returns {Promise<string | null>}
   */
  async workflowState(workflow) {
    const r = await this.request('GET', `${this.base}/actions/workflows/${encodeURIComponent(workflow)}`, { allow404: true })
    return r.status === 404 ? null : (r.data?.state ?? null)
  }
  ...
  async mergePull(number, o) {
    ...
    try {
      ...
      // Refused after a failed attempt: that attempt may have merged it.
      if (e instanceof GitHubError && e.retried && [405, 409, 422].includes(e.status)) {
        const p = await this.pullRest(number)
        if (p.state === 'merged') return { merged: true, sha: p.mergeCommit }
      }
  ...
  async createRelease(o) {
    ...
    const r = await this.request('POST', `${this.base}/releases`, {
      ...
      const done = await doneBefore(e, async () => (await this.releases()).find((x) => x.tagName === o.tag) ?? null)
      if (!done) throw e
      return { data: null, done }
    })
    return 'done' in r ? r.done : GitHub.releaseFromRest(r.data)
  }
  ...
  async uploadAsset(id, file) {
    ...
    const r = await this.request('POST', `${upload}${this.base}/releases/${id}/assets?name=${encodeURIComponent(basename(file))}`, {
      ...
      const done = await doneBefore(e, async () => (await this.releases()).find((x) => x.id === id)?.assets.find((a) => a.name === basename(file)) ?? null)
      if (!done) throw e
      return { data: { browser_download_url: done.url } }
    })
  ...
  /**
   * Jobs of the latest attempt, or of all attempts.
   * @param {number} runId
   * @param {{ all?: boolean }} [o]
   * @returns {Promise<JobInfo[]>}
   */
  async jobs(runId, o = {}) {
    const list = await this.paginate(`${this.base}/actions/runs/${runId}/jobs?filter=${o.all ? 'all' : 'latest'}`, 'jobs')
    return list.map((j) => ({ id: j.id, name: j.name, status: j.status, conclusion: j.conclusion }))
  }

  /**
   * Annotations of a job; the action reports its result as an annotation titled `release-tools`. An error is never
   * taken for "no annotations": the result decides whether a Release may be created.
   * @param {number} jobId
   * @returns {Promise<{ title: string, message: string, level: string }[]>}
   */
  async annotations(jobId) {
    const list = await this.paginate(`${this.base}/check-runs/${jobId}/annotations`)
