 ...
 * @property {number[]} runIds runs: the finished runs listed
 * @property {boolean} tool the tag was the tool's (always, except for runs: one of them shows a trace of the tool)
 */
...
async function collect(ctx, project, now) {
  ...
  for (const r of refs) {
    ...
    items.push({ name: r.name, kind, date: tag.taggerDate, refSha: tag.refSha, releaseId: release?.id ?? null, runIds: [], tool: true })
  }
  for (const rel of releases) {
    const info = classifyVersion(rel.tagName)
    if (!info || info.kind !== 'dev' || names.has(rel.tagName)) continue
    items.push({ name: rel.tagName, kind: 'dev', date: new Date(rel.createdAt), refSha: null, releaseId: rel.id, runIds: [], tool: true })
  }
  ...
  for (const run of await ctx.gh.workflowRuns(ctx.settings.releaseWorkflow)) {
    ...
    // A tag is restored only for a run of the tool. Runs of an unreleased version go whoever's they were: a run that
    // shows no trace of the tool (its tag deleted before it looked) could still release the tag pushed again.
    const tool = await project.toolRun(run)
    if (kind === 'restore' && !tool) continue
    const item = byName.get(run.headBranch) ?? { name: run.headBranch, kind, date: run.createdAt, refSha: null, releaseId: null, runIds: [], tool: false }
    item.tool ||= tool
...
async function remove(ctx, project, item) {
  ...
  if (check && (await project.isReleased(item.name))) {
    if (!item.tool) {
      ctx.ui.warn(`${item.name} was released meanwhile, not by the tool; skipped`)
      return false
    }
