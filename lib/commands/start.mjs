...
import { releaseBranches, mainVersion, unreleasedInMain, releasedUnmerged, commitLines, folderState, sameFolderState, losses, copiesOf, inMainFolder } from './common.mjs'
...
export async function createRelease(ctx, project, branch, version, startRef) {
  ...
  const wt = (await ctx.mainGit.worktrees()).find((w) => w.branch === branch && existsSync(w.path))
...
async function removeLeftover(ctx, b, into) {
  if (inMainFolder(ctx, b)) {
    ctx.ui.warn(`${b.branch} is checked out in the main folder ${ctx.mainDir}; switch it to main (git switch main) and remove it with release:start`)
    return
  }
  if (b.worktree && existsSync(b.worktree)) {
    const s = await folderState(ctx.git.at(b.worktree), b.remote ? `refs/remotes/origin/${b.branch}` : null, copiesOf(ctx))
    if (s.tracked.length || s.untracked.length || s.changedCopies.length || s.unverifiable || s.ahead || s.mergeHead) {
...
async function deleteLeftover(ctx, project, b, released) {
  if (inMainFolder(ctx, b)) {
    throw new ReleaseError(`${b.branch} is checked out in the main folder ${ctx.mainDir}`, { hint: 'switch it to main (git switch main) and run release:start again' })
  }
  void project
  void released
  const against = b.kind === 'release' ? MAIN : `${TAGS_NS}${b.version}`
  ...
  const wt = wg ? await folderState(wg, upstream, copiesOf(ctx)) : null
  if (wt?.unverifiable) throw new ReleaseError(`${b.worktree} has changes in a submodule, which the tool cannot check`, { hint: 'commit or discard them first' })
  ...
  const nowWt = wg ? await folderState(wg, upstream, copiesOf(ctx)) : null
