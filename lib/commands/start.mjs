...
import { releaseBranches, mainVersion, unreleasedInMain, releasedUnmerged, commitLines, folderState, sameFolderState } from './common.mjs'
...
async function removeLeftover(ctx, b, into) {
  ...
  if (local && (await into.isAncestor(local, 'HEAD'))) await ctx.mainGit.deleteBranch(b.branch, local)
  const remote = await ctx.gh.branchSha(b.branch)
  if (remote && (await ctx.mainGit.hasObject(remote)) && (await into.isAncestor(remote, 'HEAD'))) await ctx.mainGit.deleteRemoteBranch(b.branch, remote)
...
async function deleteLeftover(ctx, project, b, released) {
  ...
  // Only what was shown: anything new since then stays.
  if (b.worktree && existsSync(b.worktree)) {
    const now = await folderState(ctx.git.at(b.worktree), b.remote ? `refs/remotes/origin/${b.branch}` : null)
    if (wt && !sameFolderState(wt, now)) throw new ReleaseError(`${b.worktree} changed after it was shown; nothing is deleted`, { hint: 'run release:start again' })
    await ctx.mainGit.worktreeRemove(b.worktree, true)
  }
  await ctx.mainGit.worktreePrune()
  const local = await ctx.mainGit.commitOf(`refs/heads/${b.branch}`)
  if (local) {
    if (local !== b.local) throw new ReleaseError(`the local branch ${b.branch} changed after it was shown; it stays`)
