...
export async function folderState(git, upstream) {
  ...
  return { tracked: st.tracked, untracked: st.untracked, unmerged: st.unmerged, operation, mergeHead, ahead, behind, head }
}

/**
 * The state of a folder has not changed since `before` (nothing new to lose appeared during the questions).
...
export function sameFolderState(before, now) {
  ...
  return (
    ...
    now.mergeHead === before.mergeHead &&
    now.ahead === before.ahead &&
    same([...now.tracked].sort(), [...before.tracked].sort()) &&
    same([...now.untracked].sort(), [...before.untracked].sort())
  )
}
