...
export async function releaseBranches(ctx) {
  ...
  // A folder deleted by hand leaves a stale entry: pruned, so the branch is seen without a folder.
  await ctx.mainGit.worktreePrune()
  for (const w of await ctx.mainGit.worktrees()) {
    if (!w.branch || !existsSync(w.path)) continue
 ...
 * @param {{ paths: string[], mainDir: string } | null} [copies] files copied into the folder (worktree.copy): they
 *   are ignored by git, so `git worktree remove` would delete them silently when they were changed in the folder
 */
export async function folderState(git, upstream, copies = null) {
  const st = await git.status()
  /** @type {string[]} */
  const changedCopies = []
  for (const p of copies?.paths ?? []) {
    const here = join(git.dir, p)
    if (!existsSync(here)) continue
    const there = join(copies?.mainDir ?? '', p)
    const same = existsSync(there) && (await readFile(here)).equals(await readFile(there))
    if (!same) changedCopies.push(p)
  }
  ...
  const head = await git.commitOf('HEAD')
  // mergeHead: any unfinished merge, rebase, cherry-pick or revert, or conflicting entries in the index.
  const mergeHead = !!operation || st.unmerged.length > 0
  const fp = await git.fingerprint(copies?.paths ?? [])
  return {
    tracked: st.tracked,
    ...
    head,
    fingerprint: fp.text,
    // a changed submodule (or anything that is not a file or a link): the tool cannot tell what would be lost
    unverifiable: fp.unverifiable,
  }
}

/**
 * The state of a folder has not changed since `before`: the same HEAD, the same unfinished operation, and the same
 * uncommitted and untracked content (by hash), so nothing new to lose appeared during the questions.
 * @param {Awaited<ReturnType<typeof folderState>>} before
 * @param {Awaited<ReturnType<typeof folderState>>} now
 */
export function sameFolderState(before, now) {
  // Not `ahead`: it drops when the tool itself deletes the branch on GitHub; the same HEAD holds the same commits.
  if (before.unverifiable || now.unverifiable) return false
  return now.head === before.head && now.operation === before.operation && now.fingerprint === before.fingerprint
}

/**
 * Everything a folder holds that deleting it would lose, as lines to show before asking.
 * @param {import('../git.mjs').Git} git
 * @param {Awaited<ReturnType<typeof folderState>>} s
 * @param {string | null} upstream
 */
export async function losses(git, s, upstream) {
  const lost = []
  if (s.tracked.length) lost.push(`uncommitted changes:\n  ${s.tracked.join('\n  ')}`)
  if (s.unmerged.length) lost.push(`conflicts that are not resolved:\n  ${s.unmerged.join('\n  ')}`)
  if (s.untracked.length) lost.push(`files git does not track:\n  ${s.untracked.slice(0, 30).join('\n  ')}${s.untracked.length > 30 ? '\n  …' : ''}`)
  if (s.changedCopies.length) lost.push(`files copied into the folder and changed there (git ignores them):\n  ${s.changedCopies.join('\n  ')}`)
  if (s.operation) lost.push(`${s.operation} that is not finished`)
  if (s.unverifiable) lost.push('changes in a submodule (or another entry that is not a file), which the tool cannot check')
  if (upstream && s.ahead) lost.push(`commits not pushed:\n${(await commitLines(git, `${upstream}..HEAD`)).join('\n')}`)
  return lost
}
...
/**
 * The checks of a release folder that change nothing: no unfinished merge, no uncommitted changes, no committed
 * conflict markers, not diverged from its branch on GitHub.
 * @param {Context} ctx
 * @param {import('../git.mjs').Git} git
 * @param {string} branch
 */
export async function checkFolder(ctx, git, branch) {
  const upstream = `refs/remotes/origin/${branch}`
  const s = await folderState(git, upstream)
  if (s.mergeHead) {
    throw new ReleaseError(`${s.operation ?? 'a merge'} is not finished in ${git.dir}${s.unmerged.length ? ` (conflicts: ${s.unmerged.join(', ')})` : ''}`, {
  ...
  if (remote && remote !== head) {
    ...
    if (!(await git.isAncestor(head, remote)) && !(await git.isAncestor(remote, head))) {
...
/**
 * The checks of a release folder, then a fast-forward to the commits on GitHub; nothing is pushed.
 * @param {Context} ctx
 * @param {import('../git.mjs').Git} git
 * @param {string} branch
 * @returns {Promise<string | null>} the head of the branch on GitHub
 */
export async function fastForward(ctx, git, branch) {
  const { remote, head } = await checkFolder(ctx, git, branch)
  if (remote && remote !== head && (await git.isAncestor(head, remote))) {
    ...
    // The commits from GitHub are checked too (conflict markers).
    await checkFolder(ctx, git, branch)
  }
  return remote
}

/**
 * The checks of a release folder before anything changes; fast-forwards when possible and pushes local commits.
 * @param {Context} ctx
 * @param {import('../git.mjs').Git} git
 * @param {string} branch
 */
export async function prepareFolder(ctx, git, branch) {
  const remote = await fastForward(ctx, git, branch)
...
/**
 * The branch is checked out in the main folder (switched there by hand): that folder is never removed.
 * @param {Context} ctx
 * @param {{ worktree: string | null }} b
 */
export function inMainFolder(ctx, b) {
  return !!b.worktree && resolve(b.worktree) === resolve(ctx.mainDir)
}

/**
 * The files `worktree.copy` puts into release folders.
 * @param {Context} ctx
 */
export function copiesOf(ctx) {
  return { paths: ctx.settings.worktreeCopy ?? [], mainDir: ctx.mainDir }
}
...
/**
 * The release workflow is enabled on GitHub: a tag of a disabled one never gets a run.
 * @param {Context} ctx
 */
export async function checkWorkflowEnabled(ctx) {
  const state = await ctx.gh.workflowState(ctx.settings.releaseWorkflow)
  // Only a disabled workflow stops: "deleted" is what GitHub may keep after main was rewritten, while a tag still
  // runs the workflow of its commit.
  if (state !== null && state.startsWith('disabled')) {
    throw new ReleaseError(`the release workflow ${ctx.settings.releaseWorkflow} is ${state} on GitHub; no tag would get a run`, {
      hint: `enable it (Actions → ${ctx.settings.releaseWorkflow} → Enable workflow) and run the command again`,
    })
  }
}
...
export async function finalCommit(ctx, git, version) {
  ...
  const st = await git.status()
  // A new index file is untracked: it is part of the final commit as well.
  const files = [...st.tracked, ...st.untracked.filter((f) => f === build.changelogIndex)]
...
export async function unreleasedInMain(ctx, released) {
  ...
  for (const p of pulls) {
    // Only a merge that is in main now (not one of an earlier main, for example a sandbox reset).
    if (p.state === 'merged' && p.mergeCommit && (await ctx.git.hasObject(p.mergeCommit)) && (await ctx.git.isAncestor(p.mergeCommit, MAIN))) return { version: v, pr: p }
  }
  return null
}
...
/**
 * The pull request still merges `head` into `base`: GitHub lets anyone change the base, and a merge goes to the
 * current one.
 * @param {import('../github.mjs').PullInfo} p
 * @param {{ head: string, base?: string }} o
 */
export function checkPullTarget(p, o) {
  const base = o.base ?? 'main'
  if (p.baseRef !== base || p.headRef !== o.head) {
    throw new ReleaseError(`${p.url} now merges ${p.headRef} into ${p.baseRef}, not ${o.head} into ${base}`, {
      hint: `change it back to ${o.head} → ${base} (or close it) and run the command again`,
    })
  }
}

/**
 * Merges a pull request after checking again what it merges into what.
 * @param {Context} ctx
 * @param {number} number
 * @param {{ sha: string, title: string, head: string, base?: string }} o
 */
export async function mergeChecked(ctx, number, o) {
  checkPullTarget(await ctx.gh.pull(number), o)
  return ctx.gh.mergePull(number, { sha: o.sha, title: o.title })
}
 ...
 * @param {Context} ctx
 * @param {number} number
 * @param {{ head: string, base?: string, allowBypass?: boolean, expectHead?: string, bypassConfirmed?: boolean }} options head, base:
 *   the branches the pull request must merge (base main by default); bypassConfirmed: the bypass was confirmed for
 *   this release before its tag (merge after publishing without asking again)
...
export async function waitMergeable(ctx, number, options) {
  ...
  for (;;) {
    ...
    let text
    // The bypass is offered only when just the approval is missing.
    if (s === 'BLOCKED' && p.reviewDecision === 'REVIEW_REQUIRED') {
      if (options.bypassConfirmed && (await ctx.gh.canBypass(number))) return { state: 'mergeable', pull: p, bypassedBy: ctx.login }
    ...
    } else if (s === 'BLOCKED' && p.reviewDecision === 'CHANGES_REQUESTED') {
      text = `changes are requested in ${p.url}; waiting`
    } else if (s === 'BLOCKED') {
      text = `${p.url} is blocked by the rules of the repository (for example an extra approval for unattributed changes); waiting`
    } else if (s === 'DRAFT') {
