...
export async function folderState(git, upstream) {
  ...
  const fingerprint = await git.fingerprint()
  return { tracked: st.tracked, untracked: st.untracked, unmerged: st.unmerged, operation, mergeHead, ahead, behind, head, fingerprint }
}
