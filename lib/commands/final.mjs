...
import { lastStable, lastOfLine, candidateFor, stableDesc, latestLine, newerLines } from '../versions.mjs'
...
import { MAIN, Project as ProjectState } from '../project.mjs'
...
/**
 * The release pull request: the open one from the branch, else the one in the tag, else the latest merged, else the
 * latest closed. A merged or closed pull request whose head the branch does not contain belongs to an earlier
 * attempt (a release cancelled and started again) and does not count.
 * @param {Context} ctx
 * @param {Rel} rel
...
async function releasePull(ctx, rel, fromTag) {
  ...
  if (fromTag) {
    ...
    if (p) {
      checkPullTarget(p, { head: rel.branch })
      return p
    }
  }
  const head = (await ctx.gh.branchSha(rel.branch)) ?? (await ctx.mainGit.commitOf(`refs/heads/${rel.branch}`))
  const ours = async (/** @type {PullInfo} */ p) => !head || ((await ctx.git.hasObject(p.headSha)) && (await ctx.git.isAncestor(p.headSha, head)))
  const newest = all.sort((a, b) => b.number - a.number)
  for (const state of ['merged', 'closed']) {
    for (const p of newest.filter((x) => x.state === state)) if (await ours(p)) return ctx.gh.pull(p.number)
  }
  return null
}
...
async function finalChecks(ctx, project, rel, commit, o = {}) {
  ...
  if (o.early) {
    ...
    await checkWorkflowEnabled(ctx)
  } else {
  ...
  // The release pull request and the changelog pull request of a hotfix are merged with a merge commit.
  const req = await ctx.gh.mergeRequirements()
  if (req.problems.length) throw new ReleaseError(`main cannot take the merge of ${V}:\n- ${req.problems.join('\n- ')}`)
  let candidate = null
...
export async function checkHotfixAncestry(ctx, project, version, commit, released) {
  ...
  for (const versions of newerLines(version, released).values()) {
    for (const v of versions) {
      ...
      if (!t) throw new ReleaseError(`${v} is released but has no tag on GitHub, so the hotfix cannot be checked against it`, { hint: `restore the tag ${v} (release:publish offers it for versions of the tool)` })
      if ((await ctx.git.hasObject(t.commit)) && (await ctx.git.isAncestor(t.commit, commit))) {
...
export async function publishFinal(ctx, project, rel) {
  const V = rel.version
  /** @type {Set<string>} refusals of the release run seen in this command */
  ...
  for (let round = 0; round < 50; round++) {
    ...
    let pr = rel.kind === 'release' ? await releasePull(ctx, rel, tagMessage?.pr ?? null) : null
    // Merged before the final commit (main does not have the version): not a release pull request.
    if (pr?.state === 'merged' && !tag && (await mainVersion(ctx)) !== V) pr = null
    ...
    if (rel.kind === 'release' && pr?.state === 'closed') {
      ...
      if (a === 'reopen') {
        // Without a tag, a final follows, which needs the folder checked; with one, the folder is not used.
        if (!tag && (await rel.git.currentBranch()) === rel.branch) await fastForward(ctx, rel.git, rel.branch)
    ...
    if (tag) {
      const next = await handleTag(ctx, project, rel, tag, pr, invalid)
    ...
    if ((await rel.git.currentBranch()) !== rel.branch) {
      throw new ReleaseError(`${rel.git.dir} is not the folder of ${rel.branch}`, { hint: `run release:publish in the folder of ${rel.branch}` })
    }
    await fastForward(ctx, rel.git, rel.branch)
    await finalChecks(ctx, project, rel, /** @type {string} */ (await rel.git.commitOf('HEAD')), { early: true })
    if (rel.kind === 'release' && (await ctx.gh.branchSha(rel.branch))) {
      await handlePullsInto(ctx, rel.branch, pr?.state === 'open' ? pr.number : null, await otherOpenReleases(ctx, rel.branch))
    }
    ...
    if (rel.kind === 'release') {
      ...
      const w = await waitMergeable(ctx, pull.number, { head: rel.branch, expectHead: pushedHead })
 ...
 * @param {Set<string>} invalid refusals already seen in this command
 * @returns {Promise<'continue' | 'stop'>}
 */
async function handleTag(ctx, project, rel, tag, pr, invalid) {
  ...
  if (!latest) {
    ...
    // Twice in one command is no lost run: the workflow does not run on the tag.
    if (invalid.has('no-run')) {
      throw new ReleaseError(`again no release run started for the tag ${tag.name}`, {
        hint: `check that the workflow ${ctx.settings.releaseWorkflow} is enabled on GitHub and runs on the tag, then run the command again`,
      })
    }
    invalid.add('no-run')
  ...
  if (code === 'invalid-tag' || code === 'invalid-run' || code === 'no-jobs') {
    // The same outcome twice in one command is not a stale run: creating the tag again would only repeat it.
    // A run that failed before its jobs started (an invalid workflow, a GitHub outage) gets the tag created again
    // once, like a run that never started; the workflow comes from the tagged commit.
    const key = `${code}: ${latest.message ?? ''}`
    if (invalid.has(key)) {
      throw new ReleaseError(`the release run of ${tag.name} ended the same way again: ${latest.message ?? code}`, {
        hint:
          code === 'no-jobs'
            ? `look at ${latest.run.url}; ${rel.kind === 'release' ? 'fix the release workflow in the branch (merge main into it)' : 'commit the fix of the release workflow into the hotfix branch (never merge main into it)'} and run the command again`
            : latest.run.url,
      })
    }
    invalid.add(key)
  ...
  const old = (await ctx.gh.serverTime()).getTime() - latest.run.createdAt.getTime() > THIRTY_DAYS - 60 * 60 * 1000
  if (latest.failedJob === 'publish') {
    if (!old && (await artifactAvailable(ctx, latest.run.id))) {
      // One re-run of the publishing job; when it failed again (or is declined), a new build.
      if (latest.run.attempt < 2 && (await ctx.ui.confirm('Nothing reached npm. Re-run the failed publishing job?', { default: true }))) {
        await ctx.gh.rerunFailed(latest.run.id)
        await sleep(pollMs(5000))
        await project.waitForRun(tag)
        return 'continue'
      }
      if (!(await ctx.ui.confirm(`Create the tag ${tag.name} again on the same commit (a new build)?`, { default: true }))) {
...
async function mergedBeforeRelease(ctx, project, rel, pr, tag) {
  ...
  // The same checks as any final, on exactly the commit that gets the tag.
  const { candidate } = await finalChecks(ctx, project, rel, commit)
...
async function fixBranch(ctx, project, rel) {
  ...
  ctx.ui.info(`Run release:publish in ${folder}: after the approval of the pull request the tag of ${V} moves to its head.`)
  return 'stop'
}
...
export async function finishReleased(ctx, project, rel, tag) {
  ...
  if (!tag) {
    ...
    await project.restoreReleasedTag(V, null, rel.kind === 'hotfix' ? 'hotfix' : 'final')
...
export async function verifiedCommit(ctx, project, version, tag) {
  ...
  if (commit !== tag.commit) {
    ...
    // Replaced in one step, never deleted first: a failure leaves the old tag, not none.
    const kind = toolTag(tag)?.message.kind ?? 'final'
    await project.createTag({ name: version, commit, kind: /** @type {any} */ (kind), replace: tag.refSha })
    await ctx.git.fetch()
  }
...
/**
 * Stops when a run of the version found another content of it on npm: the tool never adds its GitHub Release.
 * @param {Project} project
 * @param {string} version
 */
export async function refuseMismatch(project, version) {
  const run = project.npm ? await project.integrityMismatch(version) : null
  if (run) {
    throw new ReleaseError(`${version} is on npm with another content than its release run built (${run.url}); its GitHub Release is not created`, {
      hint: `compare the package on npm with the tagged commit; if it is right, create the GitHub Release by hand (gh release create ${version} --verify-tag) and run the command again`,
    })
  }
}
...
export async function ensureRelease(ctx, project, version, commit) {
  // A run still running may find another content on npm and refuse the Release; it decides first.
  if (project.npm && !(await project.release(version))) await project.waitForRunsOf(version)
  ...
  await refuseMismatch(project, version)
  // The text comes from the changelog in the released commit: a tag restored through the API may point to a
  // commit this clone does not have yet; the tag now exists, so a fetch brings it.
  if (!(await ctx.git.hasObject(commit)) && !(await project.ensureCommit(commit))) await ctx.git.fetch()
  if (!(await ctx.git.hasObject(commit))) {
    throw new ReleaseError(`the released commit ${commit.slice(0, 12)} of ${version} cannot be fetched`, { hint: 'run the command again' })
  }
  const build = await buildSettingsAt(ctx, commit)
  ...
  return ctx.gh.createRelease({
    tag: version,
    name: version,
    body: withFooter(body, { commit, createdBy: 'release-tools CLI', kind: toolTag(tag)?.message.kind }),
    prerelease,
    latest,
    commit,
  })
...
async function mergeReleased(ctx, project, rel, H) {
  ...
  for (let round = 0; round < 30; round++) {
    ...
    if (await ctx.git.isAncestor(H, MAIN)) {
      ...
      if (pr?.state === 'merged' && pr.headSha !== H && (await ctx.git.hasObject(pr.headSha)) && (await ctx.git.isAncestor(H, pr.headSha))) {
    ...
    const w = await waitMergeable(ctx, pr.number, { head: rel.branch, expectHead: head, allowBypass: true, bypassConfirmed: confirmedBypass && head === H })
    if (w.state === 'merged' || w.state === 'closed' || w.state === 'head-changed') continue
    if (w.state === 'behind') {
      await bringUpToDate(ctx, rel, head)
      continue
    }
    const m = await mergeChecked(ctx, pr.number, { sha: head, title: `release: ${V}`, head: rel.branch })
    if (m.merged) {
...
async function bringUpToDate(ctx, rel, head) {
  if ((await rel.git.currentBranch()) !== rel.branch) {
    // Finishing from another folder: work in the folder of the release branch, never in this one.
    const b = (await releaseBranches(ctx)).find((x) => x.branch === rel.branch)
    const folder =
      b?.worktree && existsSync(b.worktree)
        ? b.worktree
        : await createRelease(ctx, new ProjectState(ctx), rel.branch, rel.version, b?.local ? `refs/heads/${rel.branch}` : `refs/remotes/origin/${rel.branch}`)
    rel = { ...rel, git: ctx.git.at(folder) }
  }
  const cur = /** @type {string} */ (await rel.git.commitOf('HEAD'))
  const s = await folderState(rel.git, `refs/remotes/origin/${rel.branch}`)
  if (s.tracked.length || s.mergeHead) {
    throw new ReleaseError(`the release folder ${rel.git.dir} has unfinished changes`, { hint: 'commit the resolution (or discard the changes) and run the command again' })
  }
  // A resolution committed in the folder after the last run is ahead of the head: it is pushed, not overwritten.
  if (cur !== head && !(await rel.git.isAncestor(head, cur)) && !(await rel.git.mergeFfOnly(head))) {
    throw new ReleaseError(`the release folder is not at the head of the pull request`, { hint: `bring ${rel.git.dir} to the head of ${rel.branch} and run again` })
  }
  await mergeMainAfterRelease(ctx, rel)
  const now = /** @type {string} */ (await rel.git.commitOf('HEAD'))
  if (now !== head && (await rel.git.isAncestor(head, now))) await rel.git.push([`HEAD:refs/heads/${rel.branch}`], { description: `push ${rel.branch}` })
}
...
async function mergeThroughNewPull(ctx, project, rel, H, original, confirmedBypass) {
  ...
  for (let round = 0; ; round++) {
    if (round >= 30) throw new ReleaseError(`${pr.url} could not be merged`, { hint: 'look at the pull request and run the command again' })
    await ctx.git.fetch()
    ...
    const w = await waitMergeable(ctx, pr.number, { head: branch, expectHead: cur.headSha })
    ...
    const m = await mergeChecked(ctx, pr.number, { sha: cur.headSha, title: `release: ${V}`, head: branch })
    if (m.merged) {
      ctx.ui.step(`merged ${cur.url} into main`)
      break
    }
    ctx.ui.status(`GitHub refused to merge ${cur.url}: ${'message' in m ? m.message : ''}; trying again`)
...
async function deleteMergeBranch(ctx, version) {
  ...
  if (sha && (await ctx.git.hasObject(sha)) && (await ctx.git.isAncestor(sha, MAIN))) {
    if (await ctx.mainGit.deleteRemoteBranch(branch, sha)) ctx.ui.step(`deleted ${branch}`)
  }
}
...
async function removeBranchAndFolder(ctx, rel, safe, against) {
  /** What is there now and whether any of it would be lost. */
  const inspect = async () => {
    await ctx.git.fetch()
    const b = (await releaseBranches(ctx)).find((x) => x.branch === rel.branch)
    if (!b) return null
    ...
    if (b.worktree && existsSync(b.worktree)) {
      ...
      state = await folderState(wg, b.remote ? `refs/remotes/origin/${rel.branch}` : null, copiesOf(ctx))
      if (state.tracked.length) problems.push(`uncommitted changes:\n  ${state.tracked.join('\n  ')}`)
      if (state.mergeHead) problems.push(`${state.operation ?? 'a merge'} that is not finished`)
      if (state.unverifiable) problems.push('changes in a submodule, which the tool cannot check')
    ...
    return { b, problems, state }
  }
  const first = await inspect()
  if (!first) return
  if (inMainFolder(ctx, first.b)) {
    ctx.ui.warn(`${rel.branch} is checked out in the main folder ${ctx.mainDir}; switch it to main (git switch main) and run release:publish again to remove the branch`)
    return
  }
  const keep = (/** @type {string[]} */ problems) =>
    ctx.ui.warn(`${rel.branch} and its folder stay, because they hold:\n${problems.join('\n')}\nrelease:start offers them as a leftover of ${rel.version}.`)
  if (first.problems.length) return keep(first.problems)
  if (first.b.remote && rel.kind === 'release') await handlePullsInto(ctx, rel.branch, null, await otherOpenReleases(ctx, rel.branch))
  let force = false
  // Untracked files and changed copies of worktree.copy (ignored by git, removed with the folder in any case).
  const untracked = [...(first.state?.untracked ?? []), ...(first.state?.changedCopies ?? [])]
  if (untracked.length) {
    ctx.ui.info(`Files that git does not track (or ignores) in ${first.b.worktree}:\n  ${untracked.slice(0, 50).join('\n  ')}${untracked.length > 50 ? '\n  …' : ''}`)
    force = await ctx.ui.confirm('Delete them together with the folder?', { default: false })
    if (!force) ctx.ui.warn(`the folder ${first.b.worktree} stays because of those files; delete it yourself when you no longer need them`)
  }
  // Checked again right before deleting: the questions may have taken a while.
  const now = await inspect()
  if (!now) return
  if (now.problems.length) return keep(now.problems)
  const b = now.b
  const unchanged =
    b.local === first.b.local && b.remote === first.b.remote && b.worktree === first.b.worktree && (!now.state || (first.state && sameFolderState(first.state, now.state)))
  if (!unchanged) {
    ctx.ui.warn(`${rel.branch} or its folder changed meanwhile; they stay. Run release:publish again to clean them up.`)
    return
  }
  let folderGone = !(b.worktree && existsSync(b.worktree))
  if (!folderGone && (!untracked.length || force)) {
    await ctx.mainGit.worktreeRemove(/** @type {string} */ (b.worktree), force)
    ctx.ui.step(`removed the folder ${b.worktree}`)
    folderGone = true
  }
  if (b.local && folderGone) await ctx.mainGit.deleteBranch(rel.branch, b.local)
  if (b.remote) {
    if (await ctx.mainGit.deleteRemoteBranch(rel.branch, b.remote)) ctx.ui.step(`deleted ${rel.branch} on GitHub`)
    else if (await ctx.gh.branchSha(rel.branch)) ctx.ui.warn(`${rel.branch} on GitHub changed meanwhile; it stays`)
  }
}
...
async function hotfixChangelog(ctx, project, V, H) {
  ...
  for (let round = 0; round < 20; round++) {
    ...
    const w = await waitMergeable(ctx, pr.number, { head: branch, expectHead: pr.headSha })
    if (w.state === 'behind') {
      const commit = await changelogCommit(ctx, V, content, mainBuild, mainPath)
    ...
    if (w.state !== 'mergeable') continue
    const m = await mergeChecked(ctx, pr.number, { sha: w.pull.headSha, title: `docs: changelog ${V}`, head: branch })
