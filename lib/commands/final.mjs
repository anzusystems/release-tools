...
async function removeBranchAndFolder(ctx, rel, safe, against) {
  ...
  const inspect = async () => {
    ...
    if (b.worktree && existsSync(b.worktree)) {
      ...
      state = await folderState(wg, b.remote ? `refs/remotes/origin/${rel.branch}` : null)
  ...
  let force = false
  const untracked = first.state?.untracked ?? []
  if (untracked.length) {
    ctx.ui.info(`Files that git does not track in ${first.b.worktree}:\n  ${untracked.slice(0, 50).join('\n  ')}${untracked.length > 50 ? '\n  …' : ''}`)
