...
async function handleTag(ctx, project, rel, tag, pr, invalid) {
  ...
  if (code === 'invalid-tag' || code === 'invalid-run') {
    // The same refusal twice in one command is not a stale run: creating the tag again would only repeat it.
    const key = `${code}: ${latest.message ?? ''}`
    if (invalid.has(key)) throw new ReleaseError(`the release run refused the tag ${tag.name} again: ${latest.message ?? code}`, { hint: latest.run.url })
    invalid.add(key)
    return moveTag(ctx, project, rel, tag, pr)
  }
  if (code === 'no-jobs') {
    throw new ReleaseError(`the release run of ${tag.name} failed before its jobs started (for example an invalid release workflow)`, {
      hint: `look at ${latest.run.url}, fix the workflow in main, and run the command again`,
    })
  }
