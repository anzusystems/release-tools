 ...
 * @param {Rel} rel
 * @param {string} commit the commit that gets the tag
 * @returns {Promise<{ candidate: string | null }>}
 */
async function finalChecks(ctx, project, rel, commit) {
...
export async function publishFinal(ctx, project, rel) {
  ...
  for (let round = 0; round < 50; round++) {
    ...
    // No tag: prepare the branch, the pull request, wait until it may be merged, check and tag.
...
async function handleTag(ctx, project, rel, tag, pr) {
  ...
  if (branchHead && branchHead !== tag.commit) {
...
async function mergeReleased(ctx, project, rel, H) {
  ...
  for (let round = 0; round < 30; round++) {
    ...
    if (await ctx.git.isAncestor(H, MAIN)) {
      ...
      if (pr?.state === 'merged' && pr.headSha !== H) {
        const extra = await commitLines(ctx.git, `${H}..${pr.headSha}`)
        const foreign = []
        for (const l of extra) {
          const sha = l.trim().split(' ')[0]
          if (!(await isMainCommit(ctx, sha))) foreign.push(l)
        }
        if (foreign.length) ctx.ui.warn(`these commits reached main without being in ${V}; they go out with the next release:\n${foreign.join('\n')}`)
      }
    ...
    const w = await waitMergeable(ctx, pr.number, { expectHead: head, allowBypass: true })
 ...
 * @param {string} sha
 */
async function isMainCommit(ctx, sha) {
  return ctx.git.isAncestor(sha, MAIN)
}
...
export async function onlyMainMerges(ctx, H, head, o) {
  ...
  const commits = (await ctx.git.log(`${H}..${head}`, ['--not', MAIN])).reverse()
  const ok = new Set([H])
