...
export async function checkHotfixAncestry(ctx, project, version, commit, released) {
  ...
  for (const [line, versions] of newerLines(version, released)) {
    ...
    if (!seen) throw new ReleaseError(`no released version of the line ${line} has a tag on GitHub, so the hotfix cannot be checked against it`, { hint: 'restore the tags (release:publish offers it)' })
  }
}
