...
async function unfinishedItems(ctx, project, released, bootstrap, branches) {
  ...
  for (const t of await project.tagRefs()) {
    ...
    if (isReleased && (!rel || rel.draft)) {
      const kind = info.kind === 'stable' ? (isOlderLine(t.name, released) ? 'hotfix' : 'release') : 'prerelease'
      items.push({
...
export async function prerelease(ctx, project, o) {
  ...
  if (branch.startsWith('hotfix/')) await checkHotfixAncestry(ctx, project, core, commit, released)
