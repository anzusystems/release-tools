...
import { folderState, sameFolderState, losses, handlePullsInto, releaseBranches, commitLines, unreleasedInMain } from './common.mjs'
...
export async function cancelRelease(ctx, project, b) {
  ...
  if (b.worktree && existsSync(b.worktree)) {
    ...
    confirmed = await folderState(wg, upstream)
  ...
  const assertUnchanged = async () => {
    if (b.worktree && existsSync(b.worktree)) {
      const now = await folderState(ctx.git.at(b.worktree), upstream)
