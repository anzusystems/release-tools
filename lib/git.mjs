...
export class Git {
  ...
  async fingerprint() {
    ...
    const paths = [...new Set([...st.tracked, ...st.untracked, ...st.unmerged])].filter((p) => existsSync(join(this.dir, p))).sort()
    let hashes = ''
    if (paths.length) {
      hashes = (await this.raw(['hash-object', '--no-filters', '--stdin-paths'], { input: `${paths.join('\n')}\n` })).stdout
    }
    return `${status}\n${paths.join('\n')}\n${hashes}`
  }
  ...
  async deleteBranch(name, expect = null) {
    ...
    if (expect && (await this.objectOf(`refs/heads/${name}`)) !== expect) {
      throw new ReleaseError(`the local branch ${name} changed meanwhile; it stays`)
    }
    // `git branch -D`, not update-ref: git refuses a branch that a folder has checked out or is rebasing.
    const r = await this.raw(['branch', '-D', name], { allowFail: true })
    if (r.code !== 0) throw new ReleaseError(`the local branch ${name} was not deleted: ${(r.stderr || r.stdout).trim()}`)
  }
