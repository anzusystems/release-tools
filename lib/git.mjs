...
export class Git {
   ...
   */
  async fingerprint() {
    const status = (await this.raw(['status', '--porcelain=v2', '-z', '--untracked-files=all'])).stdout
    // A submodule with changes cannot be checked by content here: such a folder is never deleted.
    let unverifiable = status.split('\0').some((part) => /^[12u] /.test(part) && (part.split(' ')[2] ?? '').startsWith('S'))
    const st = await this.status()
    const parts = []
    for (const p of [...new Set([...st.tracked, ...st.untracked, ...st.unmerged])].sort()) {
      const full = join(this.dir, p)
      let entry
      try {
      ...
      // lstat, not stat: a symbolic link counts by its own text, also when its target does not exist.
      if (entry.isSymbolicLink()) parts.push(`${p}\0L\0${readlinkSync(full)}`)
      else if (entry.isFile()) parts.push(`${p}\0F${entry.mode & 0o111 ? 'x' : ''}\0${createHash('sha256').update(readFileSync(full)).digest('hex')}`)
      else {
        parts.push(`${p}\0D`)
        unverifiable = true
      }
    }
    return { text: `${status}\0\0${parts.join('\0')}`, unverifiable }
  }
  ...
  async branchInUse(name) {
    for (const w of await this.worktrees()) {
      ...
      for (const f of ['rebase-merge/head-name', 'rebase-apply/head-name']) {
        ...
        if (r.code === 0 && existsSync(full) && readFileSync(full, 'utf8').trim() === `refs/heads/${name}`) return w.path
      }
    }
    return null
  }
  ...
  async deleteBranch(name, expect = null) {
    ...
    const r = expect ? await this.raw(['update-ref', '-d', `refs/heads/${name}`, expect], { allowFail: true }) : await this.raw(['branch', '-D', name], { allowFail: true })
