...
export class Project {
  ...
  /**
   * Whether a finished run ran for a tag of the tool: the action reported a result other than "nothing", or a
   * publish job of it ran. Only such a run stands for a tag of the tool that is gone; a run of a foreign tag is never
   * a reason to restore, list or delete anything.
   * @param {RunInfo} run
   */
  async toolRun(run) {
    if (run.event !== 'push' || ACTIVE.includes(run.status)) return false
    const r = await this.results(run)
    return r.published || [...r.codes].some((c) => c !== 'nothing')
  }
