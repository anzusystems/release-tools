...
export class Project {
  ...
  /**
   * Whether a finished run ran for a tag of the tool: the action reported a result other than "nothing", or its
   * publish job ran. Only such a run stands for a tag of the tool that is gone; a run of a foreign tag is never a
   * reason to restore, list or delete anything.
   * @param {RunInfo} run
   */
  async toolRun(run) {
    if (run.event !== 'push' || ACTIVE.includes(run.status)) return false
    const o = await this.outcome(run)
    if (o.code) return o.code !== 'nothing'
    return (await this.gh.jobs(run.id)).some((j) => j.name === 'publish' && !!j.conclusion && j.conclusion !== 'skipped')
  }

  /**
   * The finished run of a version that found another content of it on npm, if any: the tool never adds the
   * GitHub Release of such a version.
   * @param {string} name
   */
  async integrityMismatch(name) {
    for (const r of await this.gh.tagRuns(this.settings.releaseWorkflow, name)) {
      ...
      const o = await this.outcome(r)
      if (o.code === 'integrity-mismatch') return o
    }
    return null
  }
