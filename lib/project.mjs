...
import { lastStable, isOlderLine } from './versions.mjs'
...
/**
 * Bootstrap lasts until the tool released its first stable version, and then it is over for good: a missing tag of
 * the last released version is the state "released, tag missing", not a new bootstrap. The first stable release of
 * the tool is recognized by its GitHub Release (the footer the tool writes) or by a tag of the tool (final or
 * hotfix) of any released stable version. The same definition serves the CLI and the action.
 * @param {import('./github.mjs').GitHub} gh
 * @param {Set<string>} released
 */
export async function bootstrapOf(gh, released) {
  const stable = new Set([...released].filter((v) => semver.isStable(v)))
  if (!stable.size) return true
  for (const r of await gh.releases()) {
    if (stable.has(r.tagName) && parseFooter(r.body)?.commit) return false
  }
  for (const t of await gh.tagRefs()) {
    if (t.type !== 'tag' || !stable.has(t.name)) continue
    const tag = await gh.tag(t.name)
    const tt = tag ? toolTag(tag) : null
    if (tt && (tt.message.kind === 'final' || tt.message.kind === 'hotfix')) return false
  }
  return true
}
...
export class Project {
  ...
  async releasedCommit(version, options = {}) {
    if (this.npm) {
      ...
      if (c === undefined) throw new ReleaseError(`the provenance of ${version} on npm could not be read yet`, { hint: 'run the command again in a few minutes' })
  ...
  async bootstrap(released) {
    return bootstrapOf(this.gh, released ?? (await this.released()))
  }
  ...
  async outcome(run) {
    ...
    // Only a skipped build job marks the run of a ref deletion; a run without jobs failed to start (startup_failure).
    if (build?.conclusion === 'skipped') return { run, state: 'ref-deletion', code: null, message: null, failedJob: null }
    if (!build) return { run, state: 'failure', code: 'no-jobs', message: `the run ended (${run.conclusion}) before its jobs started`, failedJob: null }
    let code = null
    let message = null
  ...
  /**
   * The results the action reported in any attempt of a finished run, and whether a publish job of it ran.
   * @param {RunInfo} run
   */
  async results(run) {
    const codes = new Set()
    let published = false
    for (const j of await this.gh.jobs(run.id, { all: true })) {
      if (j.name === 'publish' && j.conclusion && j.conclusion !== 'skipped') published = true
      for (const a of await this.gh.annotations(j.id)) {
        const m = a.title === 'release-tools' ? /^([a-z-]+)(?::|$)/.exec(a.message.trim()) : null
        if (m) codes.add(m[1])
      }
    }
    return { codes, published }
  }

  /**
   * Whether a finished run ran for a tag of the tool: the action reported a result it reports only after it found
   * a tag of the tool, or a publish job of it ran. Only such a run stands for a released version of the tool whose
   * tag is gone; a run of a foreign tag is never a reason to restore a tag or list a release.
   * @param {RunInfo} run
   */
  async toolRun(run) {
    if (run.event !== 'push' || ACTIVE.includes(run.status)) return false
    const r = await this.results(run)
    return r.published || [...r.codes].some((c) => c !== 'nothing' && c !== 'unverified')
  }

  /**
   * A finished run of a version that found another content of it on npm (in any attempt), if any: the tool never
   * adds the GitHub Release of such a version.
   * @param {string} name
   * @returns {Promise<RunInfo | null>}
   */
  async integrityMismatch(name) {
    for (const r of await this.gh.tagRuns(this.settings.releaseWorkflow, name)) {
      if (r.event !== 'push' || ACTIVE.includes(r.status)) continue
      if ((await this.results(r)).codes.has('integrity-mismatch')) return r
    }
    return null
  }

  /**
   * Waits until no run of the tag name is waiting or running.
   * @param {string} name
   */
  async waitForRunsOf(name) {
    for (;;) {
      const active = (await this.gh.tagRuns(this.settings.releaseWorkflow, name)).filter((r) => ACTIVE.includes(r.status))
      if (!active.length) return
      this.ctx.ui.status(`waiting for the release run of ${name} to finish: ${active[0].url}`)
   ...
   * replace: the tag object the tag points to now; it is replaced in one step (never a moment without a tag).
   * A commit that is not in the local repository (for example a released hotfix whose branch is gone) is fetched by
   * its sha; when GitHub does not serve it, the tag is created through the API.
   * @param {{ name: string, commit: string, kind: 'final' | 'hotfix' | 'prerelease' | 'dev', pr?: number, candidate?: string, confirmedBy?: string, replace?: string }} t
  ...
  async createTag(t) {
    ...
    let sha
    if (await this.ensureCommit(t.commit)) {
      const content = mktagContent({ commit: t.commit, name: t.name, tagger: this.ctx.identity, epochSeconds: now.getTime() / 1000, message })
      sha = await this.git.mktag(content)
      const pushed = await this.git.push([`${sha}:refs/tags/${t.name}`], {
        allowFail: true,
        leases: t.replace ? [`refs/tags/${t.name}:${t.replace}`] : [],
        description: `push the tag ${t.name}`,
      })
      if (!pushed) {
        throw new ReleaseError(`the tag ${t.name} could not be pushed; someone may have created or changed it meanwhile`, { hint: 'run the command again' })
      }
    } else {
      const current = await this.gh.tag(t.name)
      if ((current?.refSha ?? null) !== (t.replace ?? null)) {
        throw new ReleaseError(`the tag ${t.name} was created or changed meanwhile`, { hint: 'run the command again' })
      }
      sha = await this.gh.createApiTag(t.name, t.commit, message, !!t.replace, { ...this.ctx.identity, date: now })
    }
  ...
  /**
   * The commit is in the local repository, fetched by its sha when needed.
   * @param {string} commit
   */
  async ensureCommit(commit) {
    if (await this.git.hasObject(commit)) return true
    await this.git.raw(['fetch', '--quiet', '--no-tags', '--refmap=', '--no-prune', '--no-prune-tags', '--no-recurse-submodules', '--no-write-fetch-head', 'origin', commit], {
      allowFail: true,
    })
    return this.git.hasObject(commit)
  }
   ...
   * The kind comes from the saved tag, else from the caller, else from the footer of the GitHub Release (the action
   * writes it), else from the version: a stable version of an older line is taken for a hotfix.
   * @param {string} name
   * @param {string | null} savedRef
   * @param {'final' | 'hotfix' | 'prerelease' | null} [kindHint]
   */
  async restoreReleasedTag(name, savedRef, kindHint = null) {
    ...
    if (savedRef) {
      ...
      kind = parseTagMessage(msg.split('\n\n').slice(1).join('\n\n'))?.kind ?? null
    }
    kind ??= kindHint
    kind ??= parseFooter((await this.release(name))?.body)?.fields.kind ?? null
    if (info?.kind === 'prerelease') kind = 'prerelease'
    else if (info?.kind === 'dev') kind = 'dev'
    else if (kind !== 'final' && kind !== 'hotfix') kind = isOlderLine(name, await this.released()) ? 'hotfix' : 'final'
  ...
  async recoverMoves() {
    for (const r of await this.git.refs(MOVING_NS)) {
      ...
      // A tag on GitHub means the move (or the restore) was finished, maybe by another computer: its runs are left
      // alone. Without a tag, the runs are waited for (and waiting ones cancelled) before the state is checked.
      if (!(await this.gh.tag(name))) {
        await this.stopRuns(name)
        if (!(await this.gh.tag(name)) && (await this.isReleased(name))) await this.restoreReleasedTag(name, r.ref)
      }
