...
export async function createContext(o) {
  ...
  // Pushes go to the push URLs of origin (remote.origin.pushurl may differ from the fetch URL): all of them must be
  // this repository, otherwise branches and tags would land elsewhere while the checks read GitHub.
  const pushUrls = (await git.raw(['remote', 'get-url', '--push', '--all', 'origin'], { allowFail: true })).stdout.split('\n').filter(Boolean)
  for (const u of pushUrls) {
    const r = repoFromUrl(u)
    if (r ? r.toLowerCase() !== repo.toLowerCase() : u !== url) {
      throw new ReleaseError(`origin pushes to ${u}, not to ${repo}`, { hint: 'remove remote.origin.pushurl or point it to the repository' })
    }
  }
